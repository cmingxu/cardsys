...
module ReportsHelper
  ...
  def display_income_report(date, pay_ways)
    ...
    table <<("<caption><h1>#{current_client.site_name}#{date.to_s(:db)}收入日报表"+ "</h1></caption>")
    # first tr
    table <<("<tr class='head'>")
  ...
  def display_month_income_report(date, selected_way, pay_ways)
    ...
    table = ''
    table << "<table class='report_table' border=1>"
    table <<("<caption><h1>" + current_client.site_name +  "#{date.strftime("%y-%m")}收入月报表"+ "</h1></caption>")
    # first tr%
    table <<("<tr class='head'>")
    ...
    # data tr
    CustomDateForReport.new(date, current_client.financial_begin_day_of_every_month).each_day_of_this_financial_month do |day|
      current_date = day
      table << "<tr class='report_item'>"
