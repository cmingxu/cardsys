...
module OrdersHelper
  ...
  def display_enable_buttons(order, f = nil)
    ...
    if  current_user.menus.include?("删除场地预订")
      htmls << order_action_button("取消预订", "", "cancel") if !order.balanced? and !order.activated?
  ...
  def court_status_in_period(date, start_hour, court)
    return content_tag(:td, "场地不可用") unless court.is_useable_in_time_span?(current_client.period_by_date_and_start_hour(date, start_hour))
