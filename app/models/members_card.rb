...
class MembersCard < ActiveRecord::Base
  ...
  def has_less_count?
    (card.is_counter_card? || card.is_zige_card?) && left_times < self.client.minimum_warn_count
  end

  def has_less_fee?
    (!card.is_counter_card? || card.is_zige_card?) && left_fee < self.client.minimum_warn_amount
  end
